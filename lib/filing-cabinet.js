/* eslint-disable no-sync,max-depth,complexity,max-statements*/
...
function resolveDependencyPath({dependency, filename, directory}) {
	const filepath = dependency.startsWith('.') ?
	...
	if (fileExt === '.js' && depExt !== '.js' && dependency.indexOf('!') < 0) {
		return filepath + fileExt;
	}

	if (depExt.indexOf('!') > -1) {
		return filepath + depExt.substring(0, depExt.indexOf('!'));
	}
...
module.exports = function cabinet(options) {
	...
	const resolver = defaultLookups[ext] ?? resolveDependencyPath;
...
function getCompilerOptionsFromTsConfig(tsConfig) {
	...
	if (typeof tsConfig === 'string') {
		try {
			...
			return ts.parseJsonSourceFileConfigFileContent(tsParsedConfig, ts.sys, path.dirname(tsConfig)).options;
		} catch (e) {
	...
	if (tsConfig) {
		return ts.convertCompilerOptionsFromJson(tsConfig.compilerOptions).options;
	}
...
function resolveWebpackPath({dependency, filename, directory, webpackConfig}) {
	...
	const resolveConfig = Object.assign({}, webpackConfig.resolve);
	...
	try {
		...
		const lookupPath = path.isAbsolute(dependency) ? directory : path.dirname(filename);
